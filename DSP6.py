...
def bandpass_filter(data, lowcut, highcut, fs, order=20):
    nyquist = 0.1 * fs  # Nyquist frequency
...
highcut = 0.5  # Upper cutoff frequency (Hz)
...
# Create a figure with 3 subplots (one under the other)
plt.figure(figsize=(12, 12))

# Subplot 1: Original Phase Signal
plt.subplot(3, 1, 1)  # 3 rows, 1 column, 1st subplot
plt.plot(range(len(phi)), phi, label="Original Phase", linewidth=1.5, color='blue')
...
# Subplot 2: Bandpassed Phase Signal
plt.subplot(3, 1, 2)  # 3 rows, 1 column, 2nd subplot
plt.plot(range(len(phi_bandpassed)), phi_bandpassed, label="Bandpassed Phase", linewidth=1.5, color='orange')
plt.xticks(np.arange(0, len(phi_bandpassed), step=100))  # Set x-axis ticks at intervals of 100
plt.xlabel('Frame Index (sweeps)')
plt.ylabel('Phase (radians)')
plt.title('Bandpassed Phase Signal (0.1 Hz - 0.3 Hz)')
plt.grid(True)
plt.legend()

# Subplot 3: Smoothed Phase Signal (Moving Average)
plt.subplot(3, 1, 3)  # 3 rows, 1 column, 3rd subplot
plt.plot(range(len(phi_smoothed)), phi_smoothed, label="Smoothed Phase (Moving Average)", linewidth=1.5, color='green')
plt.xticks(np.arange(0, len(phi_smoothed), step=100))  # Set x-axis ticks at intervals of 100
plt.xlabel('Frame Index (sweeps)')
plt.ylabel('Phase (radians)')
plt.title('Smoothed Phase Signal (Moving Average Filter)')
plt.grid(True)
plt.legend()

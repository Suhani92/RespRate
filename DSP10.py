    ...
    #     Loads the IQ data from the .h5 file.
    #     Computes the magnitude, finds the peak range bin, and downsamples the data.
    #     Applies a temporal low-pass filter and calculates the phase signal.
    # Clutter Suppression and Bandpass Filtering:
    #     Subtracts the mean phase to remove clutter.
    #     Applies a bandpass filter (0.1–0.5 Hz) to isolate the respiration component.
    #     Normalizes the filtered phase signal.
    # Applies EEMD:
    #     Decomposes the normalized phase signal into IMFs using EEMD.
    #     Reconstructs the respiration signal by summing the relevant IMFs (e.g., IMF 3–5).
    # Estimates the Breath Rate:
    #     Uses peak detection (time-domain) and FFT (frequency-domain) to calculate the breath rate.
    # Visualizes the Results:
    #     Plots the normalized phase signal.
    #     Plots the IMFs (optional, for visualization)
    #     Plots the reconstructed respiration signal.
    #     Plots the respiration signal with detected peaks (time-domain analysis).
    #     Plots the frequency spectrum (frequency-domain analysis).
...
plt.figure(figsize=(12, 6))
plt.plot(range(len(respiration_signal)), respiration_signal, linewidth=1.5)
...
# Time-Domain Analysis (Peak Detection)
...
def estimate_breath_rate_freq_domain(signal, fs):
    # Perform FFT on the respiration signal
    n = len(signal)
    frequencies = fftfreq(n, d=1/fs)
    fft_values = fft(signal)
    # Find the dominant frequency in the respiration band (0.1–0.5 Hz)
    respiration_mask = (frequencies >= 0.1) & (frequencies <= 0.5)
    ...
    return breath_rate, frequencies, fft_values

# Estimate breath rate using both methods
breath_rate_time_domain, peaks = estimate_breath_rate_time_domain(respiration_signal, fs)
breath_rate_freq_domain, frequencies, fft_values = estimate_breath_rate_freq_domain(respiration_signal, fs)

print(f"Breath Rate (Time Domain): {breath_rate_time_domain:.2f} bpm")
print(f"Breath Rate (Frequency Domain): {breath_rate_freq_domain:.2f} bpm")

# Plot the respiration signal with detected peaks (Time-Domain Analysis)
plt.figure(figsize=(12, 6))
plt.plot(range(len(respiration_signal)), respiration_signal, label='Respiration Signal', linewidth=1.5)
plt.plot(peaks, respiration_signal[peaks], 'ro', label='Peaks')
plt.xticks(np.arange(0, len(respiration_signal), step=100))  # Set x-axis ticks at intervals of 100
plt.xlabel('Frame Index (sweeps)')
plt.ylabel('Amplitude')
plt.title('Respiration Signal with Detected Peaks')
plt.legend()
plt.grid(True)
plt.tight_layout()  # Ensure no clipping
plt.show()

# Plot the frequency spectrum (Frequency-Domain Analysis)
respiration_mask = (frequencies >= 0.1) & (frequencies <= 0.5)
plt.figure(figsize=(12, 6))
plt.plot(frequencies[respiration_mask], np.abs(fft_values[respiration_mask]), linewidth=1.5)
plt.xlabel('Frequency (Hz)')
plt.ylabel('Amplitude')
plt.title('Frequency Spectrum of Respiration Signal')
plt.grid(True)
plt.tight_layout()  # Ensure no clipping
plt.show()